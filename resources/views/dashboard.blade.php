...
<x-slot name="header">
    <div class="flex items-center justify-between w-full gap-4">
        ...
        <button type="button"
                ...
                class="hidden lg:inline-flex items-center gap-1.5 px-3 py-2
...
<style>
    ...
    /* Icon dot */
    .icon-dot { width:34px; height:34px; border-radius:9px; flex-shrink:0;
    ...
    .dt th { padding:10px 16px; text-align:left; font-size:10px; font-weight:700;
    ...
    /* Status badge */
    .badge-active   { background:#dcfce7; color:#15803d; padding:3px 10px; border-radius:999px;
                      font-size:11px; font-weight:700; display:inline-flex; align-items:center; gap:4px }
    .badge-inactive { background:#f1f5f9; color:#64748b; padding:3px 10px; border-radius:999px;
                      font-size:11px; font-weight:700; display:inline-flex; align-items:center }
    .dark .badge-active   { background:rgba(22,163,74,.15); color:#4ade80 }
    .dark .badge-inactive { background:rgba(255,255,255,.06); color:#64748b }

    /* ID badge */
    .id-badge { font-family:monospace; font-size:11px; font-weight:700;
                color:#94a3b8; background:#f1f5f9; padding:2px 6px; border-radius:5px }
    .dark .id-badge { background:rgba(255,255,255,.06); color:#475569 }

    /* Email chip */
    .email-chip { font-size:12px; color:#6366f1; font-weight:600 }
    .dark .email-chip { color:#a5b4fc }

    /* Action buttons */
    .act-btn { padding:6px; border-radius:8px; transition:background .15s, color .15s;
               display:inline-flex; align-items:center; justify-content:center; border:none;
               background:transparent; cursor:pointer }
    .act-view { color:#64748b }
    .act-view:hover { background:#eef2ff; color:#4f46e5 }
    .dark .act-view { color:#475569 }
    .dark .act-view:hover { background:rgba(99,102,241,.15); color:#a5b4fc }
    .act-del { color:#94a3b8 }
    .act-del:hover { background:#fee2e2; color:#dc2626 }
    .dark .act-del { color:#475569 }
    .dark .act-del:hover { background:rgba(220,38,38,.15); color:#f87171 }

    /* Form inputs */
    .f-input { width:100%; border-radius:10px; border:1px solid #e2e8f0;
               background:#f8fafc; padding:8px 12px; font-size:13px; color:#1e293b;
    ...
    .dark .f-input:focus { border-color:#6366f1 }
    .f-input::placeholder { color:#94a3b8 }
    .f-label { display:block; font-size:11px; font-weight:700; letter-spacing:.06em;
               text-transform:uppercase; color:#64748b; margin-bottom:5px }
    .dark .f-label { color:#475569 }
    .f-error { font-size:11px; color:#dc2626; margin-top:4px }
    .dark .f-error { color:#f87171 }

    /* Submit button */
    .btn-submit { width:100%; display:inline-flex; align-items:center; justify-content:center; gap:6px;
                  padding:9px 18px; border-radius:10px; font-size:13px; font-weight:700;
                  background:#4f46e5; color:#fff; border:none; cursor:pointer; transition:background .15s }
    .btn-submit:hover { background:#4338ca }
    .btn-submit:active { transform:scale(.97) }

    /* FAB mobile */
    .fab { position:fixed; bottom:24px; right:24px; z-index:9999;
    ...
    /* Empty state */
    .empty-icon { width:44px; height:44px; border-radius:14px; background:#f1f5f9;
...
<div class="page-bg"
     x-data="{ users: @js($movimientos) }">
<div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-6">

    {{-- Layout: tabla + form lateral --}}
        ...
        <div class="lg:col-span-8 xl:col-span-9">
                        ...
                        <tbody>
                            <template x-for="user in users" :key="user.id">
                                <tr>
                                    ...
                                    <td>
                                        <span class="email-chip" x-text="user.email"></span>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-right">
                                        <div class="inline-flex items-center gap-1 justify-end">
                                            ...
                                            <button type="button" class="act-btn act-view" title="Ver usuario"
                                                    @click="$store.ui.selectedUser = user; $store.ui.open = false; $store.ui.openView = true">
                                            ...
                                            <form method="POST" :action="`/users/${user.id}`" class="contents">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" class="act-btn act-del" title="Eliminar usuario"
                                                        @click="
                            ...
                            <tr x-show="users.length === 0">
                                <td colspan="5" class="py-14 text-center">
                                    ...
                                    <p class="text-sm font-semibold text-gray-600 dark:text-gray-300">Sin usuarios</p>
            ...
            <div class="sm:hidden space-y-2 au d1">
                <div class="flex items-center justify-between mb-1 px-1">
                    <span class="s-label">
                        <span x-text="users.length"></span> usuario(s)
                    </span>
                </div>
            ...
            <div class="panel">
                ...
                <form method="POST" action="{{ route('users.store') }}" class="p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <input id="u-name" name="name" value="{{ old('name') }}" required
                               class="f-input" placeholder="Ej: Juan Pérez">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input id="u-pass" type="password" name="password" required
