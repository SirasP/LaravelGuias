<x-app-layout>
    ...
    <div class="page-bg premium-bg min-h-screen" x-data="{ filter: 'all' }">
        <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-7 space-y-7 premium-content">

            {{-- Stats Grid (KPI Cards) --}}
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <x-kpi-card 
                    label="Total Presupuestado" 
                    value="${{ number_format($stats->total_clp, 0) }}"
                ...
                <x-kpi-card 
                    ...
                    :pct="$stats->total > 0 ? ($stats->confirmed / $stats->total * 100) : 0"
                >
                ...
                <x-kpi-card 
                    ...
                    :pct="$stats->total > 0 ? ($stats->sent / $stats->total * 100) : 0"
                >
                    <svg class="w-4 h-4 text-indigo-600 dark:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg>
                </x-kpi-card>

                <x-kpi-card 
                    label="Presupuesto USD" 
                    value="US${{ number_format($stats->total_usd, 0) }}"
                    iconBg="bg-amber-50 dark:bg-amber-900/20"
                    :trend="0"
                >
                    <svg class="w-4 h-4 text-amber-600 dark:text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path></svg>
                </x-kpi-card>
            </div>
            ...
            <div class="panel au d1">
                ...
                <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between gap-3 flex-wrap">
                    ...
                    <span class="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Historial de gestiones</span>
                </div>
