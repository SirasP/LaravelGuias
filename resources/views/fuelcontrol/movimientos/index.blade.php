...
<div id="modalXml" 
     ...
     onclick="cerrarModalSiFondoClick(event)">
    
    <div class="bg-white dark:bg-gray-900 w-full max-w-6xl max-h-[90vh] rounded-2xl shadow-2xl transform transition-all duration-300 scale-95 opacity-0"
         id="modalContent"
         onclick="event.stopPropagation()">
        
        <!-- Header del Modal -->
        <div class="sticky top-0 z-10 bg-gradient-to-r from-blue-600 to-blue-700 dark:from-blue-700 dark:to-blue-800 px-6 py-4 rounded-t-2xl border-b border-blue-500/20">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-blue-100 mt-0.5" id="modalSubtitle">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button 
                        ...
                        class="hidden group relative px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-all duration-200 text-sm font-medium"
                        id="btnDescargar">
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button 
                        ...
                        class="group relative px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-all duration-200 text-sm font-medium"
                        id="btnImprimir">
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button 
                        onclick="cerrarModal()" 
                        class="group relative w-10 h-10 bg-white/10 hover:bg-red-500 text-white rounded-lg transition-all duration-200 flex items-center justify-center">
        ...
        <!-- Loader -->
        <div id="modalLoader" class="flex items-center justify-center py-20">
            <div class="text-center">
                <div class="inline-block animate-spin rounded-full h-12 w-12 border-4 border-gray-200 border-t-blue-600 mb-4"></div>
                <p class="text-gray-500 dark:text-gray-400 text-sm">Cargando documento...</p>
            </div>
        </div>

        <!-- Contenido del Modal -->
        <div id="contenidoXml" class="hidden overflow-y-auto" style="max-height: calc(90vh - 80px);">
            <!-- Aquí se carga el XML -->
        </div>

        <!-- Footer del Modal -->
        <div class="sticky bottom-0 bg-gray-50 dark:bg-gray-800 px-6 py-3 rounded-b-2xl border-t border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                <div class="flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    <span>Sistema FuelControl - Documentos Tributarios</span>
                </div>
...
<style>
    ...
    #contenidoXml::-webkit-scrollbar {
        width: 8px;
    }

    #contenidoXml::-webkit-scrollbar-track {
        background: #f1f5f9;
        border-radius: 4px;
    }

    #contenidoXml::-webkit-scrollbar-thumb {
        background: #cbd5e1;
        border-radius: 4px;
    }

    #contenidoXml::-webkit-scrollbar-thumb:hover {
        background: #94a3b8;
    }
...
<script>
    ...
    function abrirMovimiento(id) {
        ...
        const contenido = document.getElementById('contenidoXml');
        ...
        // Cargar XML
        fetch(`/fuelcontrol/movimientos/${id}/xml`)
            ...
            .then(data => {
                ...
                if (data.folio) {
                    subtitle.textContent = `Folio N° ${data.folio} - ${data.tipo || 'Documento Tributario'}`;
                }
                
                // Mostrar botón descargar si hay XML
                if (xmlActual) {
                ...
                contenido.innerHTML = `
                    <div class="flex items-center justify-center py-20">
                        <div class="text-center">
                            ...
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Error al cargar el documento</h3>
                            <p class="text-sm text-gray-500 dark:text-gray-400">No se pudo cargar el XML. Por favor, intenta nuevamente.</p>
                            <button onclick="cerrarModal()" class="mt-4 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors">
                                Cerrar
                            </button>
                        </div>
                    </div>
    ...
    function cerrarModalSiFondoClick(event) {
        if (event.target.id === 'modalXml') {
            cerrarModal();
        }
    }
    ...
    function descargarXML() {
        ...
        const blob = new Blob([xmlActual], { type: 'application/xml' });
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `documento_${Date.now()}.xml`;
    ...
    observer.observe(document.getElementById('modalXml'), {
        attributes: true,
