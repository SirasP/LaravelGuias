<x-app-layout>
    ...
    <style>
        ...
        .kpi-box { background:#f8fafc; border:1px solid #f1f5f9; border-radius:14px; padding:14px 16px }
        .dark .kpi-box { background:#0f1623; border-color:#1e2a3b }
        .stat-label { font-size:10px; font-weight:700; letter-spacing:.06em; text-transform:uppercase; color:#94a3b8; margin-bottom:3px }
        .mov-row { border-radius:14px; border:1px solid #f1f5f9; background:#fafafa; overflow:hidden; transition:.15s }
        .mov-row:hover { border-color:#e2e8f0 }
        .dark .mov-row { background:#0f1623; border-color:#1e2a3b }
        .dark .mov-row:hover { border-color:#2d3a52 }
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
            ...
            <div class="panel">
                ...
                <div class="px-5 pt-5 pb-4">
                    ...
                    {{-- KPI grid --}}
                    <div class="grid grid-cols-2 {{ $isVenta ? 'sm:grid-cols-3 lg:grid-cols-5' : 'sm:grid-cols-3' }} gap-3">
                        <div class="kpi-box">
                            <p class="stat-label">Movimientos</p>
                            <p class="text-2xl font-black text-gray-900 dark:text-gray-100">{{ $summary->movimientos }}</p>
                        </div>
                        <div class="kpi-box">
                            <p class="stat-label">Unidades totales</p>
                            <p class="text-2xl font-black text-gray-900 dark:text-gray-100">{{ number_format((float)$summary->cantidad_total, 0, ',', '.') }}</p>
                        </div>
                        <div class="kpi-box" style="background:#fff1f2; border-color:#fecdd3">
                            <p class="stat-label" style="color:#f43f5e">Costo total</p>
                            <p class="text-2xl font-black text-rose-600 dark:text-rose-400">$ {{ number_format((float)$summary->costo_total, 0, ',', '.') }}</p>
                        </div>
                        @if ($isVenta)
                            ...
                            <div class="kpi-box col-span-2 sm:col-span-1 {{ $margenTotal !== null && $margenTotal >= 0 ? 'bg-emerald-50 dark:bg-emerald-900/20' : ($margenTotal !== null ? 'bg-rose-50 dark:bg-rose-900/20' : '') }}">
                                <p class="stat-label">Margen</p>
                                <p class="text-2xl font-black {{ $margenTotal !== null ? ($margenTotal >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600') : 'text-gray-400' }}">
                                    {{ $margenTotal !== null ? $margenTotal.'%' : '—' }}
                                </p>
                                @if ($isVenta && $summary->sin_precio > 0)
                                    <p class="text-[11px] text-amber-500 font-semibold mt-0.5">{{ $summary->sin_precio }} sin precio</p>
                                @endif
                            </div>
                        @endif
                    </div>
                            ...
                            <div class="mov-row" x-data="{ open: false }">
                                ...
                                <div class="px-4 py-3 flex items-center justify-between gap-3 cursor-pointer"
                                    ...
                                    <div class="flex items-center gap-3 min-w-0">
                                        <div class="shrink-0 w-9 h-9 rounded-xl {{ $isVenta ? 'bg-emerald-100 dark:bg-emerald-900/30' : ($isEpp ? 'bg-blue-100 dark:bg-blue-900/30' : 'bg-gray-100 dark:bg-gray-800') }} flex items-center justify-center">
                                            <span class="text-base leading-none">{{ $isVenta ? '💰' : ($isEpp ? '🦺' : '📦') }}</span>
                                        </div>
                                        <div class="min-w-0">
                                            <p class="text-sm font-bold text-gray-900 dark:text-gray-100">
                                                {{ \Carbon\Carbon::parse($m->ocurrio_el)->format('d/m/Y') }}
                                            </p>
                                            <p class="text-xs text-gray-400">#{{ $m->id }} · {{ $mLines->count() }} {{ $mLines->count() === 1 ? 'producto' : 'productos' }}</p>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-4 shrink-0">
                                        <div class="text-right hidden sm:block">
                                            <p class="text-xs text-gray-400">Costo</p>
                                            <p class="text-sm font-bold text-rose-600 dark:text-rose-400 tabular-nums">$ {{ number_format($mCosto, 0, ',', '.') }}</p>
                                        </div>
                                        @if ($isVenta && $mPv > 0)
                                            <div class="text-right hidden sm:block">
                                                <p class="text-xs text-gray-400">Venta</p>
                                                <p class="text-sm font-bold text-emerald-600 dark:text-emerald-400 tabular-nums">$ {{ number_format($mPv, 0, ',', '.') }}</p>
                                            </div>
                                        @endif
                                        @if ($mMg !== null)
                                            <span class="hidden sm:inline text-xs font-bold px-2 py-0.5 rounded-full {{ $mMg >= 0 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-rose-100 text-rose-700' }}">
                                                {{ $mMg }}%
                                            </span>
                                        @endif
                                ...
                                <div x-show="open" x-collapse x-cloak class="border-t border-gray-100 dark:border-gray-800">
                                    ...
                                    <div class="sm:hidden px-4 pt-3 grid grid-cols-2 gap-2">
                                        <div class="bg-rose-50 dark:bg-rose-900/20 rounded-xl px-3 py-2">
                                            ...
                                            <p class="text-sm font-bold text-rose-600 dark:text-rose-400">$ {{ number_format($mCosto, 0, ',', '.') }}</p>
                                        </div>
                                        @if ($isVenta)
                                            <div class="bg-emerald-50 dark:bg-emerald-900/20 rounded-xl px-3 py-2">
                                                <p class="text-[10px] text-emerald-500 uppercase tracking-wide font-bold">Venta</p>
                                                <p class="text-sm font-bold text-emerald-600 dark:text-emerald-400">{{ $mPv > 0 ? '$ '.number_format($mPv, 0, ',', '.') : '—' }}</p>
                                            </div>
                                        @endif
                                    </div>
                                                    ...
                                                    @foreach($mLines as $line)
                                                        <tr>
                                                            <td class="py-1.5 pr-2">
                                                                <p class="font-semibold text-gray-800 dark:text-gray-200">{{ $line->producto }}</p>
                                                                <p class="text-gray-400">{{ $line->unidad }}</p>
                                                            </td>
                                                            <td class="py-1.5 pr-2 text-right tabular-nums text-gray-600 dark:text-gray-300">
                                                                {{ number_format((float)$line->cantidad, 2, ',', '.') }}
                                                            </td>
                                                            <td class="py-1.5 text-right tabular-nums font-semibold text-gray-800 dark:text-gray-200">
                                        ...
                                        <div class="sm:hidden px-4 pt-2 space-y-1.5">
                                            @foreach($mLines as $line)
                                                <div class="flex items-center justify-between text-xs">
                                                    <span class="text-gray-700 dark:text-gray-300 font-medium truncate mr-2">{{ $line->producto }}</span>
                                                    <span class="tabular-nums text-gray-500 shrink-0">{{ number_format((float)$line->cantidad, 2, ',', '.') }} · $ {{ number_format((float)$line->costo_total, 0, ',', '.') }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    @endif
                            ...
                            <div class="space-y-1.5">
                                <div class="flex items-center justify-between gap-2">
                                    <div class="min-w-0 flex items-center gap-2">
                                        <span class="text-[10px] font-bold text-gray-400 w-4 text-right shrink-0">{{ $i + 1 }}</span>
                                    ...
                                    <div class="text-right shrink-0">
                                        ...
                                        <p class="text-[10px] text-gray-400 tabular-nums">$ {{ number_format((float)$p->costo, 0, ',', '.') }}</p>
                                    </div>
                                </div>
