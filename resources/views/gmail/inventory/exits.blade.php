<x-app-layout>
    ...
    <style>
        ...
        /* Section wrappers */
        .section-venta  { border-left:4px solid #10b981 }
        .section-epp    { border-left:4px solid #3b82f6 }
        .section-salida { border-left:4px solid #94a3b8 }

        /* Section header accent */
        .section-header-venta  { background: linear-gradient(135deg,#ecfdf5,#f0fdf4); border:1px solid #bbf7d0 }
        .section-header-epp    { background: linear-gradient(135deg,#eff6ff,#f0f9ff); border:1px solid #bfdbfe }
        .section-header-salida { background: linear-gradient(135deg,#f8fafc,#f1f5f9); border:1px solid #e2e8f0 }
        .dark .section-header-venta  { background:#063320; border-color:#064e3b }
        .dark .section-header-epp    { background:#0c1f40; border-color:#1e3a5f }
        .dark .section-header-salida { background:#111827; border-color:#1e2a3b }
        ...
        .name-group-header {
            ...
            text-transform:uppercase; color:#64748b; margin:12px 0 8px;
        }
        .name-group-header::after {
            content:''; flex:1; height:1px; background:#e2e8f0;
        }
        .dark .name-group-header { color:#94a3b8 }
        .dark .name-group-header::after { background:#1e2a3b }
    </style>
    ...
    <div class="page-bg">
        <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
            ...
            @else
                {{-- Sections: Venta / EPP / Salida --}}
                @php
                    $sectionMeta = [
                        'Venta'  => ['icon' => '💰', 'label' => 'Ventas',    'cls' => 'venta'],
                        'EPP'    => ['icon' => '🦺', 'label' => 'EPP',       'cls' => 'epp'],
                        'Salida' => ['icon' => '📦', 'label' => 'Salidas',   'cls' => 'salida'],
                    ];
                @endphp

                @foreach ($sectionOrder as $tipo)
                    @if (($stats[$tipo]['count'] ?? 0) === 0) @continue @endif
                    @php
                        $meta       = $sectionMeta[$tipo];
                        $byName     = $grouped->get($tipo, collect());
                        $secStats   = $stats[$tipo];
                    @endphp

                    <div x-data="{ open: true }"
                         class="rounded-2xl overflow-hidden section-{{ $meta['cls'] }}
                                bg-white dark:bg-gray-900/30 border border-gray-200 dark:border-gray-800">

                        {{-- Section header (collapsible) --}}
                        <button type="button" @click="open = !open"
                            class="w-full section-header-{{ $meta['cls'] }} px-5 py-4
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="text-left">
                                    ...
                                    <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">
                                        {{ $secStats['count'] }} {{ $secStats['count'] === 1 ? 'movimiento' : 'movimientos' }}
                                        &middot;
                                        Costo: $ {{ number_format((float) $secStats['costo'], 0, ',', '.') }}
                                        @if ($tipo === 'Venta' && (float) $secStats['venta'] > 0)
                                            &middot;
                                            Venta: $ {{ number_format((float) $secStats['venta'], 0, ',', '.') }}
                                        @endif
                                    </p>
                                </div>
                            </div>
                        ...
                        <div x-show="open" x-collapse class="px-5 pb-5">
                            ...
                            @foreach ($byName as $nombre => $movs)
                                ...
                                <div class="name-group-header">
                                    {{ $nombre }}
                                    <span class="text-[10px] font-semibold text-gray-400 normal-case tracking-normal ml-1">
                                        ({{ $movs->count() }})
                                    </span>
                                </div>

                                {{-- Cards grid for this name --}}
                                    ...
                                    @foreach ($movs as $m)
                                        ...
                                        <div class="exit-card"
                                            ...
                                            <div class="px-4 pb-4 pt-2 border-t border-gray-100 dark:border-gray-800 space-y-2">
                                                ...
                                                {{-- Only show sell button for Venta type --}}
                                                @if ($tipo === 'Venta')
